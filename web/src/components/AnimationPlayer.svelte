      ...
      <div class="scene-scroll">
        {#if script.kind === 'array'}
          <svg class="scene" viewBox={`0 0 ${Math.max(300, 68 + arrayData.length * 62)} 162`} role="img" aria-label="数组状态">
            ...
            {#key index}{#each arrayData as value, i}
              <g transform={`translate(${34 + i * 62}, 62)`}><g class:changed={changedArray(i)} class="scene-item">
        ...
        {:else if script.kind === 'grid'}
          <svg class="scene" viewBox={`0 0 ${Math.max(300, 64 + Math.max(...gridData.map((row) => row.length)) * 58)} ${72 + gridData.length * 50}`} role="img" aria-label="网格状态">
            ...
            {#key index}{#each gridData as row, r}{#each row as value, col}
              <g transform={`translate(${34 + col * 58},${38 + r * 50})`}><g class:changed={changedGrid(r, col)} class="scene-item">
        ...
        {:else if script.kind === 'list'}
          <svg class="scene" viewBox={`0 0 ${Math.max(310, 72 + arrayData.length * 88)} 170`} role="img" aria-label="链表状态">
            ...
            {#key index}{#each arrayData as value, i}
              ...
              <g transform={`translate(${34 + i * 88},62)`}><g class:changed={changedArray(i)} class="scene-item">
        ...
        {:else}
          <svg class="scene" viewBox={`0 0 ${tree.width} ${tree.height}`} role="img" aria-label="二叉树状态">
            ...
            {#key index}{#each tree.nodes as node (node.id)}<g transform={`translate(${tx(node)},${ty(node)})`}><g class="scene-item"><circle r="19" fill={treeFill(node.id)} stroke={treeFill(node.id) === palette.base ? palette.stroke : '#e45a68'} stroke-width="2.3"/><text y="5" text-anchor="middle" fill={palette.ink} font-size="15" font-family="monospace">{node.value}</text></g></g>{/each}{/key}
          </svg>
        {/if}
      </div>
